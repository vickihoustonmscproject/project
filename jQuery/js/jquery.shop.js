	...
	$.Shop.prototype = {
		...
		displayCart: function() {
			if( this.$formCart.length ) {
				...
				if( items.length == 0 ) {
					$tableCartBody.html( "<tr><td colspan='3'>Your order is currently empty. Please choose a dish from the menu.<br>&nbsp;</td></tr>" );
				} else {
					...
					for( var i = 0; i < items.length; ++i ) {
						...
						if (addons.length!==0) {
							html += "<br><em>" + addons + "";
								...
								html += "<em><br>&nbsp;</td>";
								html += "<td class='pprice'>" + price + "<br>&nbsp;</td><td class='pdelete'><a href='' data-product='" + product + "'>&times;</a><br>&nbsp;</td></tr>";
		  ...
		  if( self.$updateCartBtn.length ) {
			self.$updateCartBtn.on( "click", function() {
				...
				$rows.each(function() {
					...
					var paddons = $.trim( $row.find( ".paddons" ).text() );
		...
		handleAddToCartForm: function() {
			...
			self.$formAddToCart.each(function() {
				...
				$form.on( "submit", function() {
					if ($form.data ("id")=="modalform") { //check if from modal form
								//alert ("I am from modal");
								//alert ("Mydish is "+$form.find( ".mydish" ).val());
								var name =  $form.find( ".mydish" ).val();
								var price =  self._convertString($form.find( ".myprice" ).val());
								//alert (name+''+price);
								var category = 0;
					} else { //if not from modal form
								//alert ("I am not from modal");
					...
					if (category=="1") { //if sides menu show drop down options and caluclate price
					var addons = $form.find( ".addons" ).val();
					var addonsprice = self._convertString($form.find( ".addons option:checked" ).data("addprice"));
					price+=addonsprice;
					price=price.toFixed(2);
				} else { //if ramen show ramen extras
					var addons=[];
					$.each($("input[name='addons']:checked"), function(){
						//alert('Original price :£'+price);
						//alert('Extra price :£'+$(this).data('addprice'));
						newprice=price + (self._convertString($(this).data('addprice')));
						price=newprice;
						//alert('New price is :£'+newprice);
						var addonsprice = self._convertString($(this).data('addprice'));
                addons.push($(this).val()); //Add extras to array
            });
						if (!addons || !addons.length) {
							//alert ('No addons added');
							price=price.toFixed(2);
						} else {
            //alert("Extras added are: " + addons.join(", "));
